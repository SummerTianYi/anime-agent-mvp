...
当前模型源文件没有内置动作片段，因此 MVP 使用程序化骨骼动作。动作运行时按骨骼族谱分层：`上半身` 的全部后代归入上半身层，`下半身`、腿脚、裙摆及脚部 IK 归入下半身层；不会按腰部高度切割网格。两条从颈侧出发的长辫子各有完整的 17 节骨骼链，根名为 `MaWei_R_0_1` / `MaWei_L_0_1`，全部归入上半身，并叠加逐节的轻微惯性摆动；后脑勺发束不参与这项修正。模型内置的 48 个形变键已接入眨眼、单眼、微笑、惊讶、生气、眼泪和元音口型。
