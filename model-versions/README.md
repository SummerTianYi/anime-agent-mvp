...
2026-09-10 Codex 本地状态补充：A2 已接入默认外观 1.4 并通过表情回归、重启实装，但尚未提交冻结，因此下表与索引仍保留最后正式版1.3。[当前增量、独立备份与回退](PENDING_1.4.md)。
...
| `1.3` | accepted（当前，Codex） | 与 `1.2` 字节一致 | `581a856c51b1de4387896ce5ae283e279ff67d22` | 保守 A-pose、已批准材质精修及 F12 高清截图；[验证与下一阶段](1.3/README.md) |

`1.0` 至 `1.3` 的 GLB 与可编辑 Blend 都分别保存；虽然当前文件哈希相同，仍保留四个独立副本，确保后续流程不会覆盖历史。中间未通过比例验收的实验态不在本档案中。下一次正式优化分配 1.4（BaseVersion 1.3）；下方命令的 1.3 是本次已执行示例，不可重复创建。
