...
Implemented and frozen (see `docs/HANDOFF.md` §3 for the full list with exam evidence): text-chat loop with sessions, wake word + STT pipeline, TTS sidecar client with interruption, read-only tool loop, persona prompt override, three-tier permission engine with audit, atomic `write_file` with write-roots + in-chat confirmation, `run_command` with command whitelist, per-tool-type narration, MCP stdio host with confirmation routing, proactive startup greeting with quiet hours, screen vision via GLM native multimodality (privacy ask-gate, live-verified 2026-09-07), memory facts table with tiered promotion and lexical recall. all by zcode (2026-09-05→07; see docs/CONTRIBUTIONS.md zcode rows — do not misattribute to Codex). Idle trigger (local poll + template bank, standby is zero-quota), STT wedge self-heal, and five real MCP servers (browser/GitHub/search/Gmail/Drive, 93 tools) shipped 2026-09-07. Core session supervision shipped by zcode and was hardened with startup/exit lifecycle checks by Codex on 2026-09-11; see `docs/STARTUP_REPAIR_STATUS.md`. Not implemented: Windows login autostart (owner deferred), audio/TTS isolation and historical task recovery, keyboard/mouse control (deferred by design), pending-memory review UI, Spark Adapter. `docs/TESTING.md` (203 tests) is the acceptance baseline.
