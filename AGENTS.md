...
Implemented and frozen (see `docs/HANDOFF.md` §3 for the full list with exam evidence): text-chat loop with sessions, wake word + STT pipeline, TTS sidecar client with interruption, read-only tool loop, persona prompt override, three-tier permission engine with audit, atomic `write_file` with write-roots + in-chat confirmation, `run_command` with command whitelist, per-tool-type narration, MCP stdio host with confirmation routing, proactive startup greeting with quiet hours, memory facts table with tiered promotion and lexical recall. Not implemented: Idle trigger, Windows autostart/supervision, STT self-healing, keyboard/mouse control (deferred by design), vision model configuration (`look_at_screen` degrades gracefully), pending-memory review UI, Spark Adapter. `docs/TESTING.md` (154 tests) is the acceptance baseline.
