...
> 最后核验：2026-09-12（Codex 正式表情实装；zcode 计数同步 204）；Python 单测 214 项全绿（门禁以 [TESTING.md](TESTING.md) 当前口径为准）。
...
2026-09-12 Codex 表情实装：所有者批准惊讶/生气最终候选及哭泣折中候选，现由 `emotion_presets.gd` 接入正式菜单/快捷键/Core事件；补充组合切换、口型/眨眼优先级、超时清除与底层状态恢复。模型外观仍为1.4，官模/比例/材质/动作不变；表情修订独立记为1.0。4207帧动态回归、六组GPU一致性对照、122组既有材质/表情对照、180帧口型及Core/UI/动作门禁通过。仅本地实装，未commit/push；按原CMD入口重启，旧运行时与本次实现独立归档，详见 [正式表情1.0](../apps/avatar-runtime/lookdev/EXPRESSION_1.0.md)。
...
2026-09-10 zcode 思考强度档（所有者点名"鲸鱼娘旋钮天依版"，行为层经所有者定稿）：composer"思考·大展身手"按钮 → ChatGPT 式弹窗（自绘粗轨道 + #66CCFF 已选段 + 天依蓝三档，官方表情包敲碗/吃瓜/棒做档位旋钮，已镜像统一朝向并抠底，`assets/effort/<level>.png` 只留本地，缺图回退头像）；档位显示名所有者定稿：碎碎念/帮帮忙/大展身手。协议 `chat.message` 新增可选 `effort`。**行为层定稿（所有者定调"档位=投入度而非功能开关"）：三档均为完整 agent——全注册表+全部 MCP 常驻在岗，权限引擎不因档位降级；档位只调投入度=步数阶梯 1/3/5（deep=历史默认）+ 按档行为提示注入（`EFFORT_PROMPT_NOTES`：碎碎念=单点快查短答 / 帮帮忙=小任务利落交代 / 大展身手=先规划后汇报）**。单测 191→200 全绿；守卫 `verify_effort_ui.gd`（GODOT_EFFORT_UI_OK）+ 既有三守卫回归绿；`runtime.gd send_chat_message` 仅加带默认值的参数（冻结文件最小加法）。GLM thinking 参数透传待 key 有额度后验证；长远语义登记于 [plans 手册 UI-U2a](plans/README.md)。
...
2026-09-09 Codex 倾听过渡修正（覆盖下段 v2 的当前状态）：所有者否决 corrected v2 中段“蛇形扭臂”，首尾认可。现注册为独立 `listen_reference_fk_v1.tres`，通过 `mocap/build_listen_transition.gd` 保留首尾、用固定局部旋转弧重做中段，不再逐帧重算 IK 肘平面或掌向。新增 480 子步路径门禁，旧版明确失败、新版通过；右手腕累计旋转 100.831°→11.664°，右上臂 276.265°→141.033°（局部合成旋转路程，不等于纯轴向扭转角）。全骨架/取消重入、四视角实际渲染、旧动作/输入/资产回归与 191 项隔离 Core 测试、桌面构建通过。官模、1.3 外观、语音服务不改；本次单代理，无 commit/push，待所有者自然度验收。完整证据、独立本地归档、复现和单动作回退见 [mocap/README.md](../apps/avatar-runtime/mocap/README.md) 顶部；下段为已被本轮修订的历史记录。
...
| 2 | 跑 §9 快速恢复流程第 6 步的单测基线 | 214 项全绿 |
...
| 6 | 显式 Mock、禁 TTS/wake/MCP、隔离数据目录后，在 `services/agent-core` 运行 `.venv\Scripts\python.exe -m unittest discover -s tests -v`（具体环境见 TESTING） | 214 项全绿；不触碰生产库或真实外部服务 |
