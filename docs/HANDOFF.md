...
|---|---|---|
| `apps/avatar-runtime/runtime.gd` | Window behavior, model discovery, bones, semantic motion-layer filtering, expressions, authored-track rebinding/playback, pigtail secondary sway, Core WebSocket and state/action execution | Changing 3D behavior, dragging, camera, animation layers, expressions or bridge reception; garment physics is intentionally delegated to the dedicated cloth repository |
...
| P0 | Build the character motion set | Keep the verified registry/mocap-idle baseline and produce thinking, speaking-body and greeting clips; preserve the static visual model, expressions, input and bridge. Garment collision work belongs in `SummerTianYi/anime-agent-cloth-physics` and must not be rebuilt as per-action patches here |
