...
This document is the operational source of truth for a new Agent with no access to prior conversations. The product is a local Windows 3D desktop companion whose visible surface is a Godot-rendered Luo Tianyi avatar; do not reinterpret it as a conventional Agent chat window, do not make Codex/Claude/Spark a production dependency, and do not commit the local character model or user data. Start by reading `AGENTS.md`, `docs/CONTRIBUTIONS.md` and `model-versions/README.md`, use the table below to select the relevant code, run the baseline checks in `docs/TESTING.md`, and only then modify behavior.

## Snapshot: 2026-09-01

| Area | Implemented and verified | Remaining boundary |
|---|---|---|
| Avatar | Godot working-area transparent/borderless/always-on-top canvas; fixed-optical-axis 2× SubViewport rendered into a movable transparent composite; dynamic click-through hull; click menu; bottom-overlay chat UI in the Luo Tianyi theme with scrollable bubbles and session controls; persisted left-drag, right-drag turn and wheel zoom | No installer, autostart or crash supervisor; chat UI deletes sessions via a single-click confirm dialog, fresh sessions get an LLM-generated title after the first exchange, history pagination stays out of scope |
| Model behavior | 1 skeleton; 48 expression slots; tracked motion registry; semantic upper/lower bone masks; two complete 17-bone pigtail chains classified as upper body; a 12.47-second monocular-mocap looping idle starts automatically; the 4.967-second pirouette is the full-body stress clip; the 8.533-second MP4-reference listen clip follows open-palm→fist→open, starts on `voice.state=recording`, holds while recording and returns to idle on transcription; procedural gestures and expressions remain available; 16×3 runtime skirt springs plus 20 limb proxies and panel contacts prevent verified idle/pirouette hand/leg penetration without changing the official GLB | Thinking/speaking/greeting still need authored clips; transition policy is simple cross-fade rather than an AnimationTree; pigtails have only subtle procedural secondary sway and no collision-aware physics; skirt contact is character-specific rather than general cloth self-collision, so new large clips must pass its dedicated gate |
| Model versions | Accepted `1.0` pre-optimization baseline, `1.1` fixed-optical-axis clear version and current `1.2` external Toon/color-calibrated version have independent local read-only Blend/GLB snapshots, tracked manifests and exact Git runtime commits; 1.2 includes seven-view evidence, exact 1.1 pixel rollback and OpenGL/Vulkan motion gates | Restricted binaries/evidence images stay local; future 1.3+ versions must remain immutable and use isolated worktrees for exact historical comparison |
| Chat loop | Godot sends `chat.message` with an optional `conversationId`; Core loads session-scoped history per request and calls the Provider; `chat.response` returns text/emotion/gesture plus `conversationId`; `session.new`, `session.list` and `chat.history` events cover creating, listing and reloading sessions (see `docs/AVATAR_BRIDGE.md`); `ANIME_AGENT_TOOLS` adds a step-capped read-only tool-call loop (`get_time`/`read_file`/`list_dir`/`screenshot`/`active_window`/`clipboard_read`) that degrades to plain completion when the endpoint rejects tools; GLM 5.3 Flash Coding endpoint has been used successfully | No streaming, cancellation or retries/backoff; history pagination and a manual rename UI stay out of scope |
...
|---|---|---|
| `apps/avatar-runtime/runtime.gd` | Window behavior, model discovery, bones, semantic motion-layer filtering, expressions, authored-track rebinding/playback, pigtail secondary sway, skirt spring/contact runtime, Core WebSocket and state/action execution | Changing 3D behavior, dragging, camera, animation layers, garment contacts, expressions or bridge reception |
...
| P0 | Build the character motion set | Keep the new registry/mocap-idle baseline and produce thinking, speaking-body and greeting clips; preserve the static visual model, expressions, input and bridge; add collision-aware pigtail dynamics and require every large clip to pass the existing skirt geometry gate |
| P1 | TTS | Reply audio, mouth timing and interruption are tested separately from STT |
| P2 | Permission layer and tools | Read-only capability first; mutation and external actions require explicit confirmation; blueprint in `docs/plans/AGENT_ROADMAP.md` |
