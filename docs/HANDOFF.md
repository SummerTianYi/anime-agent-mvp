...
| Model versions | Accepted `1.0` pre-optimization baseline and `1.1` fixed-optical-axis clear version have independent local read-only Blend/GLB snapshots, tracked manifests and exact Git runtime commits; scripts verify, switch assets and open isolated worktrees | Preview slots are defined but the full front/side/back/expression screenshot set must be captured before the next accepted optimization; restricted binaries stay local |
