...
| Model behavior | 1 skeleton; 48 expression slots; tracked motion registry; semantic upper/lower bone masks; two complete 17-bone pigtail chains classified as upper body; an 8.03-second looping idle starts automatically; the 4.967-second pirouette is the full-body stress clip; the 8.533-second MP4-reference listen clip follows open-palm→fist→open, starts on `voice.state=recording`, holds while recording and returns to idle on transcription; procedural gestures and expressions remain available | Thinking/speaking/greeting still need authored clips; transition policy is simple cross-fade rather than an AnimationTree; pigtails have only subtle procedural secondary sway and no collision-aware physics; skirt dynamics remain absent |
| Model versions | Accepted `1.0` pre-optimization baseline and `1.1` fixed-optical-axis clear version have independent local read-only Blend/GLB snapshots, tracked manifests and exact Git runtime commits; Codex has a local, switchable `1.2-preview` material candidate with complete front/side/back/max-zoom/neutral/expression evidence, exact 1.1 rollback and OpenGL/Vulkan motion gates | `1.2-preview` is not an accepted version and has no manifest/snapshot/commit; project-owner visual approval is mandatory before freezing 1.2, and restricted binaries/evidence images stay local |
