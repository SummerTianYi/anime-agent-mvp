...
2026-09-09 Codex 本地增量：按所有者的新视频与背手照片替换 `listen` 为 `listen_mocap_v2.tres`；74 个上半身旋转轨道，2 秒进入，录音保持，结束/提前取消沿原路径反向返回，重复触发不跳回初始帧。右手/头胸部分来自测量，遮挡的左手、掌向及轻微前倾为人工适配；不修改官模、1.3 材质、待机及 agent/语音服务。旧倾听 GLB 和新动捕源均在本地保留；已完成离线与实际 GPU 渲染验证，待所有者自然度验收，未 push，未重启生产 Core/TTS；证据、限制和回退见 [mocap/README.md](../apps/avatar-runtime/mocap/README.md)。当前 Core 实测 187 项通过，原文档仍有 181 的历史基线漂移（来自并行唤醒测试增量），不得把文档守卫列为全绿。
