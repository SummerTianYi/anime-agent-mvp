...
| Godot motion runtime | `& '..\tools\godot-4.7.2\Godot_v4.7.2-stable_win64_console.exe' --headless --path apps\avatar-runtime --script res://verify_motion_runtime.gd` | Prints `GODOT_MOTION_RUNTIME_OK`; idle loops, pirouette interrupts it, static visual model is preserved and playback returns to idle |
