...
| Godot bridge | With `relaxed_arms=true`, idle has 289 tracks (79 retained non-arm tracks + 210 single-key tracks for 70 shoulder/arm descendant bones); original idle without this policy retains 106 tracks. Pirouette stays at 124 and listen at 127. `voice.state=recording` starts listen, its final pose is held while recording, and transcribing/idle returns to authored idle |
...
| Acceptance scope | The sampled idle views separate hands from the skirt. Existing listen/pirouette interactions are unchanged and remain unapproved for collision-free production use; animation boundaries and naturalness still need owner review. |
