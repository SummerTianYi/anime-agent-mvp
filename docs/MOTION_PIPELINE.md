...
| Skirt dynamics | 16 complete three-bone chains; Godot runtime-only spring solver; 20 live hand/forearm/thigh sphere proxies; adjacent-chain panel contact solver; actual 5,464 weighted skirt vertices sampled across full idle and pirouette; official GLB remains byte-identical |
...
| `apps/avatar-runtime/runtime.gd` | Registry loading, semantic bone-layer filtering, track-path rebinding, idle autoplay, interaction interruption, pigtail secondary motion, runtime-only skirt dynamics/contact resolution, playback/cancel/reset and keyboard/bridge trigger |
| `apps/avatar-runtime/verify_skirt_physics.gd` | Full-idle/full-pirouette 30/60/120 FPS contact gate using live transforms and CPU-skinned skirt geometry; writes local visual evidence |
...
The pirouette remains a technical acceptance clip, while idle and listen are character-facing motions. Next add thinking, speaking-body and greeting clips to the registry; facial mouth shapes remain driven independently by Agent state. The double-pigtail layer covers all 34 skinned bones and still has only subtle procedural sway, while the skirt now uses runtime spring chains plus local panel contacts. New clips must pass `verify_skirt_physics.gd`; this is a character-specific approximation, not a general cloth/self-collision engine. Reference video may be supplied as the original MP4/WebM/MOV—the user does not need to split frames. MediaPipe extraction is used for captured full-body motion, while Blender frame extraction remains available for visual references; neither requires the user to split images manually.
