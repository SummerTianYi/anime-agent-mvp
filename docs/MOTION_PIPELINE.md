...
The authored-motion path now contains a tracked registry plus two local clips: an 8.03-second subtle looping idle generated from sparse Blender poses, and a CMU BVH pirouette used as a full-body stress test. Godot keeps `assets/luotianyi_v4.glb` for materials and mesh appearance, extracts animation tracks from each ignored motion GLB, filters them by semantic bone layer, rewrites their bone paths to the static skeleton and discards the generated model instances. This avoids the washed-out materials produced by Blender's generic glTF export; idle starts automatically, interactions interrupt it, and completion returns to idle.
...
| Godot bridge | 107 upper-body idle and 124 full-body pirouette tracks retained after rebinding; normal-material static model preserved; idle loops and resumes after interaction |
